...
interface TeacherTeacherFormProps {
  open: boolean;
  onClose: () => void;
  onSaved: () => void;
...
export default function TeacherTeacherForm({ open, onClose, onSaved, initialData }: TeacherTeacherFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && initialData) {
        await apiClient.put(`/teachers/${initialData.id}`, payload);
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button type="submit" form="teacher-teacher-form" loading={submitting}>Simpan</Button>
        </>
      }
    >
      <form id="teacher-teacher-form" onSubmit={handleSubmit} className="space-y-4" noValidate>
