...
export default function TeacherClassDeleteDialog({ open, onClose, onDeleted, data }: Props) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(apiError);
      toast.error("Gagal menghapus kelas.", { description: apiError.message });
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    ...
    >
      <p className="text-sm text-on-surface-variant">
        Hapus kelas <span className="font-semibold text-on-surface">{data?.name ?? ""}</span>? Tindakan ini tidak dapat dibatalkan.
      </p>
