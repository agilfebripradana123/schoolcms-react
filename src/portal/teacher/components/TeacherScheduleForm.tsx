...
export default function TeacherScheduleForm({ open, onClose, onSaved, initialData }: TeacherScheduleFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && initialData) {
        await apiClient.put(`${TEACHER_MANAGE.SCHEDULES}/${initialData.id}`, payload);
        toast.success("Jadwal berhasil diperbarui.");
      } else {
        await apiClient.post(TEACHER_MANAGE.SCHEDULES, payload);
      ...
      toast.error("Gagal menyimpan jadwal", { description: apiError.message });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      <form id="teacher-schedule-form" onSubmit={handleSubmit} className="space-y-6" noValidate>
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <FormField label="Kelas" required error={fieldErrors.class_id?.[0]}>
            {classesLoading ? (
              ...
              <div className="flex w-full flex-col gap-2 rounded-2xl border border-error/30 bg-error-container px-4 py-3 text-sm text-error">
                ...
                <Button type="button" variant="secondary" size="sm" onClick={() => { setClassesLoading(true); setClassesError(false); loadClasses(); }} className="self-start">Muat Ulang</Button>
              </div>
            ) : classes.length === 0 ? (
              <p className="rounded-2xl border border-outline-variant bg-surface-container-low px-4 py-3 text-sm text-on-surface-variant">Tidak ada kelas tersedia.</p>
            ) : (
              <AppSelect value={classId} onChange={(v) => setClassId(v ?? "")} options={classOptions} placeholder="Pilih Kelas" isDisabled={submitting} />
            )}
          </FormField>
          ...
          <FormField label="Mata Pelajaran" required error={fieldErrors.subject_id?.[0]}>
            {subjectsLoading ? (
              ...
              <div className="flex w-full flex-col gap-2 rounded-2xl border border-error/30 bg-error-container px-4 py-3 text-sm text-error">
                ...
                <Button type="button" variant="secondary" size="sm" onClick={() => { setSubjectsLoading(true); setSubjectsError(false); loadSubjects(); }} className="self-start">Muat Ulang</Button>
              </div>
            ) : subjects.length === 0 ? (
              <p className="rounded-2xl border border-outline-variant bg-surface-container-low px-4 py-3 text-sm text-on-surface-variant">Tidak ada mata pelajaran tersedia.</p>
            ) : (
              <AppSelect value={subjectId} onChange={(v) => setSubjectId(v ?? "")} options={subjectOptions} placeholder="Pilih Mata Pelajaran" isDisabled={submitting} />
            )}
          </FormField>
          ...
          <FormField label="Guru" hint="Opsional." error={fieldErrors.teacher_id?.[0]}>
            {teachersLoading ? (
              ...
              <div className="flex w-full flex-col gap-2 rounded-2xl border border-error/30 bg-error-container px-4 py-3 text-sm text-error">
                ...
                <Button type="button" variant="secondary" size="sm" onClick={() => { setTeachersLoading(true); setTeachersError(false); loadTeachers(); }} className="self-start">Muat Ulang</Button>
              </div>
            ) : teachers.length === 0 ? (
              <p className="rounded-2xl border border-outline-variant bg-surface-container-low px-4 py-3 text-sm text-on-surface-variant">Tidak ada guru tersedia.</p>
            ) : (
              <AppSelect value={teacherId} onChange={(v) => setTeacherId(v ?? "")} options={teacherOptions} placeholder="Pilih Guru" isClearable isDisabled={submitting} />
            )}
          </FormField>

          <FormField label="Hari" required error={fieldErrors.day?.[0]}>
            <AppSelect value={day} onChange={(v) => setDay((v ?? "senin") as ScheduleDay)} options={DAY_OPTIONS} isSearchable={false} isDisabled={submitting} />
          </FormField>
          ...
          <FormField label="Jam Pelajaran" required error={fieldErrors.period_id?.[0]}>
            {periodsLoading ? (
              ...
              <div className="flex w-full flex-col gap-2 rounded-2xl border border-error/30 bg-error-container px-4 py-3 text-sm text-error">
                ...
                <Button type="button" variant="secondary" size="sm" onClick={() => { setPeriodsLoading(true); setPeriodsError(false); loadPeriods(); }} className="self-start">Muat Ulang</Button>
              </div>
            ) : periods.length === 0 ? (
              <p className="rounded-2xl border border-outline-variant bg-surface-container-low px-4 py-3 text-sm text-on-surface-variant">Tidak ada jam pelajaran tersedia.</p>
            ) : (
              <AppSelect value={periodId} onChange={(v) => setPeriodId(v ?? "")} options={periodOptions} placeholder="Pilih Jam Pelajaran" isDisabled={submitting} />
            )}
          </FormField>
          ...
          <FormField label="Tahun Ajaran" required error={fieldErrors.academic_year_id?.[0]}>
            {yearsLoading ? (
              ...
              <AppSelect value={academicYearId} onChange={(v) => setAcademicYearId(v ?? "")} options={yearOptions} placeholder="Pilih Tahun Ajaran" isDisabled={submitting} />
            )}
          </FormField>
          ...
          <FormField label="Semester" hint="Opsional." error={fieldErrors.semester_id?.[0]}>
            {semestersLoading ? (
              ...
              <div className="flex w-full flex-col gap-2 rounded-2xl border border-error/30 bg-error-container px-4 py-3 text-sm text-error">
                ...
                <Button type="button" variant="secondary" size="sm" onClick={() => { setSemestersLoading(true); setSemestersError(false); loadSemesters(); }} className="self-start">Muat Ulang</Button>
              </div>
            ) : semesters.length === 0 ? (
              <p className="rounded-2xl border border-outline-variant bg-surface-container-low px-4 py-3 text-sm text-on-surface-variant">Tidak ada semester tersedia.</p>
            ) : (
              <AppSelect value={semesterId} onChange={(v) => setSemesterId(v ?? "")} options={semesterOptions} placeholder="Pilih Semester" isClearable isDisabled={submitting} />
            )}
          </FormField>
        </div>
