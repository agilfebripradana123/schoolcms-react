...
export default function GuardianListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((g) =>
      [g.name, g.relation, g.phone, g.occupation, g.address, g.student?.name, g.student?.nisn]
        .filter(Boolean)
  ...
  const handleDeleteConfirm = useCallback(async () => {
    if (!toDelete) return;
    try {
      await guardianService.remove(toDelete.id);
      toast.warning(`Data wali ${toDelete.name || ""} berhasil dihapus.`);
      setDeleteOpen(false);
      setToDelete(null);
      setLoading(true);
      setError(null);
      ...
      toast.error("Gagal menghapus data wali siswa", {
        description: toApiError(err).message,
      });
    }
  }, [toDelete, fetchList]);

  const openEdit = useCallback((row: Guardian) => {
    setEditing(row);
    setFormOpen(true);
  }, []);

  const openDelete = useCallback((row: Guardian) => {
    setToDelete(row);
    setDeleteOpen(true);
  }, []);
  ...
  const columns = useMemo(() => {
    ...
    return [
      ...
      {
        header: "Hubungan",
        accessor: "relation" as keyof Row,
        render: (_val: unknown, row: Row) => (
          <span className="text-sm text-on-surface-variant">{row.relation ?? "-"}</span>
        ),
      },
      {
        header: "Pekerjaan",
        ...
        render: (_val: unknown, row: Row) => (
          <span className="text-sm text-on-surface-variant">{row.occupation ?? "-"}</span>
        ),
      },
      {
        header: "Telepon",
  ...
  return (
    <PageContainer className="py-6">
      <PageHeader
        title="Wali Siswa"
        description="Kelola data wali / perwalian siswa."
      />
          ...
          <>
              ...
              {loading ? (
                ...
                pageData.map((row) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-on-surface-variant">{row.student?.name ?? ""}</p>
            ...
            <div className="hidden sm:block">
              <DataTable
                ...
                data={pageData}
                loading={loading}
                emptyMessage={search ? "Tidak ada wali yang cocok." : "Belum ada data wali siswa."}
              />
            </div>
          </>
        )}
      ...
      <ConfirmDialog
        ...
        description={`Apakah Anda yakin ingin menghapus data wali ${toDelete?.name || ""}? Tindakan ini tidak dapat dibatalkan.`}
        confirmText="Hapus"
        cancelText="Batal"
        destructive
