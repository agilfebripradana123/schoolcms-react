...
export default function AttendanceListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((a) =>
      [a.status, a.note, a.date, a.student?.name, a.student?.nisn, a.class?.name]
        .filter(Boolean)
  ...
  const openEdit = useCallback((row: Attendance) => {
    setEditing(row);
    setFormOpen(true);
  }, []);

  const openDelete = useCallback((row: Attendance) => {
    setToDelete(row);
    setDeleteOpen(true);
  }, []);
  ...
  const columns = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        render: (_val: unknown, row: Row) => (
          <span className="text-sm text-on-surface-variant">{row.class?.name ?? "-"}</span>
        ),
      },
      {
        header: "Catatan",
        accessor: "note" as keyof Row,
        render: (_val: unknown, row: Row) => (
          <span className="text-sm text-on-surface-variant">{row.note ?? "-"}</span>
        ),
      },
      {
        header: "Aksi",
  ...
  return (
    <PageContainer className="py-6">
      <PageHeader
        title="Kehadiran"
        description="Kelola catatan kehadiran siswa."
        actions={
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="w-full sm:max-w-xs">
            <Search
              ...
              placeholder="Cari siswa / status / tanggal..."
            />
          </div>
          ...
          <>
            ...
            <div className="space-y-3 sm:hidden">
              {loading ? (
                ...
                <div className="py-10 text-center text-slate-500">
                  {search ? "Tidak ada kehadiran yang cocok." : "Belum ada data kehadiran."}
                </div>
              ) : (
                pageData.map((row) => {
                  const meta = STATUS_META[row.status ?? ""];
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs text-on-surface-variant">{row.date ? row.date.substring(0, 10) : "-"}</p>
                        </div>
                        {meta ? (
            ...
            <div className="hidden sm:block">
              <DataTable
                ...
                data={pageData}
                loading={loading}
                emptyMessage={search ? "Tidak ada kehadiran yang cocok." : "Belum ada data kehadiran."}
              />
            </div>
          </>
        )}
      ...
      <ConfirmDialog
        ...
        description="Apakah Anda yakin ingin menghapus catatan kehadiran ini? Tindakan ini tidak dapat dibatalkan."
        confirmText="Hapus"
        cancelText="Batal"
        destructive
