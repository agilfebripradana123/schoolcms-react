...
interface ExamScheduleDeleteDialogProps {
  open: boolean;
  onClose: () => void;
  onDeleted: () => void;
  data: ExamSchedule | null;
}

export default function ExamScheduleDeleteDialog({
  open,
  onClose,
  onDeleted,
  data,
}: ExamScheduleDeleteDialogProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await examScheduleService.remove(data.id);
      onDeleted();
  ...
  return (
    ...
    >
      <p className="text-sm text-on-surface-variant">
        Apakah Anda yakin ingin menghapus jadwal ujian ini? Tindakan ini tidak
        dapat dibatalkan.
      </p>
